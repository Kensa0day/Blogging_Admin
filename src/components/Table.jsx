                    ...
                    <div className="flex gap-3 items-center">
...

                        {
                            follower.followers.length > 0 && (
                                <span className="text-sm to-slate-600 font-bold">{formatNumber(follower.followers.length)} Followers</span>
                            )
                        }
                    </div>
                </>
...
export const RecentPostTable = ({data, theme}) => {
    const tableData = data?.map((el) => (
        <Table.Tr key={el?._id} className={theme ? "text-gray-400" : "text-slate-600"}>
            <Table.Td className="flex gap-2 items-center">
